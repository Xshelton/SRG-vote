    ...
    #list_pairs=[]
       ...
       with tf.Session() as sess:
         if predict_mode=='all':
           ...
           global label_mirna
           global count
           df_gene=pd.read_csv('gene_seq_12502_34567.csv')
           label_gene=df_gene['label']
           df_gene=df_gene.drop(['label'],axis=1)
           df_mirna=pd.read_csv('mirna_Seq_2600.csv')
         ...
         #  initial_load()
           ...
           TF.to_csv('./outcome/s{}_s_result.csv'.format(i,filename[0:-4]))
           print(TF)
           print('The CSV format file has been generated')
           np.save('./outcome/sgLSTM_result'.format(filename[0:-4]),list_score)
...
list_file=['s2s-SGlstmwhole_FIVE.csv']
filename=list_file[0]
check_path()
#train_pre(filename,500,'lstm')#training and predicting
predict(filename,500,'lstm')
