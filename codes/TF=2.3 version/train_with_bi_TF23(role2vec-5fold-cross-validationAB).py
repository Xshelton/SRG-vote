...
def train_pre_fivefold(filename,mode):
   ...
   for j in range(0,1):#R2R—lstmcore_epoch_{}_1.csv
    list_result=[]
    for i in range(1,6):#R2R—lstmcore_epoch_0_{}.csv
      five_name='{}_epoch_{}_{}.csv'.format(filename[0:-4],j,i) 
      print(five_name)
      epoch=5#defalt is 500, here can set to 1 for quick test 
...
def trian_pre_discard(filename,mode):
    ...
    for j in range(0,1):#control model
        test_file='{}_discard_{}.csv'.format(filename[0:-4],j)
        
        for i in range(1,6):#control model,decide which model
           list_opt=[]
           five_name='{}_epoch_{}_{}.csv'.format(filename[0:-4],j,i) 
