    ...
    #list_pairs=[]
       ...
       with tf.compat.v1.Session() as sess:
         ...
         if predict_mode=='file':
             ...
             print(ori)
             ori.to_csv('./results/{}_test_results_{}_{}_auc{}.csv'.format(filename[0:-4],model,iteration,roc_auc),index=None)
             return roc_auc
...
def train_pre_fivefold(filename,mode):
   ...
   for j in range(0,1):#control file GCNGCN_lstmcore_epoch_{}_1.csv
    list_result=[]
    for i in range(1,6):#control epoch GCNGCN_lstmcore_epoch_0_{}.csv
      five_name='{}_epoch_{}_{}.csv'.format(filename[0:-4],j,i) 
      print(five_name)
      epoch=500#defalt is 500, here set to 1 for quick test
...
def trian_pre_discard(filename,mode):
    ...
    for j in range(0,1):#control model models_predictGCNGCN_lstmcore_epoch_{}_1_5_lstm
        test_file='{}_discard_{}.csv'.format(filename[0:-4],j)
        
        for i in range(1,6):##control epoch, decide which model models_predictGCNGCN_lstmcore_epoch_0_{}_5_lstm
           list_opt=[]
           five_name='{}_epoch_{}_{}.csv'.format(filename[0:-4],j,i) 
...
#for new cross-validation's prediction: （TypeB Only:because only type B have discarded dataset) 
#for kk in range(0,1):
#    filename=list_file[kk]
